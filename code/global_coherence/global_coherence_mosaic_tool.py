...
def myargsparse(a):
	...
	******* cache from s3 bucket, keep locally
	\r{thisprog} -n Myregion -u s3://sentinel-1-global-coherence-earthbigdata  -c /dev/shm/tiles -ullr -115 50 -100 40
	******* cache from s3 bucket, upload back to s3 bucket and delete locally
	\r{thisprog} -n NorthAmerica -u s3://sentinel-1-global-coherence-earthbigdata  -c /dev/shm/tiles -t NAtiles.txt -s3 -delete
	\r{thisprog} -n NorthAmerica -u s3://sentinel-1-global-coherence-earthbigdata  -c /dev/shm/tiles -t s3://ebd-scratch-w/jpl_coherence/north_america/NAtiles.txt -s3 -delete
	...
	p.add_argument("-u","--url_root",required=False,help='URL root for tiles, e.g. "s3://<Bucket>" (AWS/Wasabi s3 Bucket) or "/" (local filesystem), "/mnt/t" (nfs mounted file system) where tiles are stored.',action='store',default='s3://sentinel-1-global-coherence-earthbigdata')
	...
	p.add_argument("-o","--outdir",required=False,help='URL root to store generated COGs if -tr is provided. Can filesystem path or s3 url. e.g. s3://ebd-jpl ',action='store',default='s3://ebd-user-scratch/regionalvrts')
