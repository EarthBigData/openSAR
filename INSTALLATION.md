...
To create the *ebd* environment use the [conda_ebd.yml](https://github.com/EarthBigData/openSAR/tree/master/conda_ebd.yml?raww=True) file available at [https://github.com/EarthBigData/openSAR](https://github.com/EarthBigData/openSAR). On most browsers you can *Right Click*  this link to save the file to your local machine. Make sure you are downloaded the "raw" file and not the html verions. Some browsers may add a ".txt" ending, so you may have to rename the file after download to "conda_ebd.yml. "Save or move the file into the same directory path from which you will execute the command below. 
